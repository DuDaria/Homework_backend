...
class Enterprise:
    def __init__(self, name, town):
        assert isinstance(name, str) and isinstance(town, str), '*** Должно быть строкой ***'
        self.name = name
        self.town = town
        self.employees = []
        self.positions = [] 
    ...
    def hire_employee(self, people, position):
        ...
        assert isinstance(position, str), '*** Должно быть строкой ***'
        self.employee = []
    ...
    def fire_employee(self, position, id):
        ...
        assert isinstance(position, str) and isinstance(id, str), '*** Должно быть строкой ***'
    ...
    def add_code_and_sallary(position):
        ...
        elif position != "Директор" or "Бухгалтер" or "Кадровик" or "Менеджер по продукции":
            raise ValueError
...
class Person:
    def __init__(self, last_name, first_name, midl_name, birthday, phone):
        assert isinstance(phone, str) and isinstance(birthday, str) and \
               isinstance(last_name, str) and isinstance(first_name, str) and \
               isinstance(midl_name, str), '*** Должно быть строкой ***'
...
if __name__ == "__main__":
    ...
    new_person1 = Person('Здоровяк', 'Олег', 'Константинович', '01.01.1975', '8-800-2050-600')
    ...
    enterprise.get_emploee_in_position("Бухгалтер")
    # enterprise.get_emploee_in_position(None)
    print("")
